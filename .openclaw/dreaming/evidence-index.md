...
- **Evidence ID:** EV-022
- **Evidence statement:** Cycle 11's forecast predicted `main` post-merge = 127 + 1 + 1. Actual on `main` at merge SHA `fd822b0` (re-measured cleanly via `git checkout fd822b0 && make dreaming-validate`) was **130 + 1 + 1** (off by +3 passed). Root cause: cycle 11 added 1 NEW file to `.openclaw/dreaming/` — `cycle-11-review-log.md` (committed by reviewer) — and modified 2 existing files (`workflow-nightly-dreaming.md` adding Stage 11 + Stage 12, `proposed-improvements.md` adding PI-018 + PI-019). `tests/dreaming/test_no_hidden_reasoning_capture.py` has three `@pytest.mark.parametrize("path", _all_dreaming_files(), ...)` tests that enumerate every file in `.openclaw/dreaming/` and run three parametrized test invocations per file. The 1 NEW file was newly enumerated by `_all_dreaming_files()` and added +3 parametrized test invocations; the 2 modified files were already present pre-cycle-11 and did not add new parametrized test invocations. The forecast (reasoning from `def test_` count of 1 new test function for cycle 11) missed the parametrized-test expansion because the forecast methodology did not account for it. Per PI-018 / Stage 11 step 6, the cycle-11 closeout memo (`memory/2026-07-01-cycle-11-closeout.md`) was corrected with the actual measured count and a Forecast-accuracy section explaining the off-by-N. The post-merge correction commit `e9b3807` records the actual count in `pr-change-log.md`. PI-020 (cycle 12 NEW) addresses this forward-looking by adding a pre-merge collect-only baseline-capture step that makes the forecast a captured number rather than a reasoned estimate.
- **Quantitative summary:**
  ...
  - Cycle 11 added 1 NEW file to `.openclaw/dreaming/` (`cycle-11-review-log.md`, committed by reviewer) and modified 2 existing files (`workflow-nightly-dreaming.md` adding Stage 11 + Stage 12, `proposed-improvements.md` adding PI-018 + PI-019). Only the 1 NEW file was newly enumerated by `_all_dreaming_files()` and contributed +3 parametrized test invocations in `test_no_hidden_reasoning_capture.py` (3 parametrized tests × 1 newly-enumerated file); the 2 modified files were already present pre-cycle-11 and did not add new parametrized test invocations.
...
## EV-023 — Cycle-12 forecast was numerically correct as collected but labeled as passed (−2 forecast-format-labeling delta at PR #72) (NEW, cycle 13)

- **Evidence ID:** EV-023
- **Evidence statement:** Cycle 12's forecast (in `pr-change-log.md` cycle-12 row, line 645) predicted `Main post-merge (forecast): 136 passed + 1 skipped + 1 expected-fail-on-main`. The cycle-12 row's reasoning (per PI-020 + Stage 0a): branch-local collect-only baseline 133 + 3 parametrized-test expansion delta = 136 tests collected. So the cycle-12 forecast's number `136` was the **collected** count, not the **passed** count. The forecast was labeled `136 passed` — a **forecast-format labeling bug**. Actual on `main` at post-PR-#72 merge SHA `5fbc1f9` (re-measured cleanly via `make dreaming-validate` on `main`) was **134 passed + 1 skipped + 1 expected-fail-on-main** (136 tests collected − 1 skipped `test_commits_use_chore_dreaming_prefix` − 1 expected-fail-on-main `test_current_branch_uses_dreaming_prefix`). The arithmetic `136 collected → 134 passed + 1 + 1` matches the actual perfectly; the cycle-12 forecast's −2 delta was purely a label-format issue. Cycle-12 PR #71 also failed the forecast (off by −4, see cycle-12 closeout memo and PR #71 actual line in `pr-change-log.md`) but for a different reason: PR #71 was merged at Round 3 fix-up state (`a1920b3`) and did not include the cycle-12 reviewer log (committed locally in Round 4 as `ebbb3b9`, not pushed to origin), so no parametrized-test expansion occurred on `main`, and the actual matched the branch-local collect-only baseline of 133 − 1 (skipped) = 132 passed. PR #72 follow-up merged the reviewer log + drift check + regex widening, and the actual count became 134 passed + 1 skipped + 1 expected-fail-on-main (reviewer log's +3 expansion fired), revealing the label-format bug. Per PI-018 / Stage 11, the cycle-12 final closeout memo (`memory/2026-07-07-cycle-12-final-closeout.md`) was created documenting the −2 label-format delta and proposing PI-021 (forecast-format-label clarification) + PI-022 (forecast-merge-state clarification) for cycle 13.
- **Quantitative summary:**
  - Cycle-12 PR #71 (`34f3793`): forecast 136 + 1 + 1 → actual 132 + 1 + 1 → **−4 passed**. Cause: merge-state assumption wrong (reviewer log not in merge).
  - Cycle-12 PR #72 (`5fbc1f9`): forecast 136 + 1 + 1 → actual 134 + 1 + 1 → **−2 passed**. Cause: forecast-format labeling bug (136 = collected, not passed).
  - Cycle-12 collect-only baseline (branch-local pre-PR #71): 133 tests collected.
  - Cycle-12 collect-only baseline (post-PR #72): 136 tests collected (133 + 3 reviewer-log parametrized expansion).
  - Cycle-12 added 1 NEW file to `.openclaw/dreaming/` (`cycle-12-review-log.md`, committed by reviewer in PR #72) and modified 6 existing files. Only the 1 NEW file was newly enumerated by `_all_dreaming_files()` and contributed +3 parametrized test invocations in `test_no_hidden_reasoning_capture.py` (3 parametrized tests × 1 newly-enumerated file); the 6 modified files were already present pre-cycle-12 and did not add new parametrized test invocations.
  - Net-new `def test_` functions in cycle 12: +2 (`test_pr_change_log_includes_collect_only_forecast_baseline`, plus the drift check + regex widening for the cycle-12 round 5 test).
  - Total new test invocations on `main` post-PR #72: +3 (1 from new function + 0 net from PR #72's round 5 changes that were already in PR #71, plus 3 from parametrized expansion via reviewer log) = 136 collected.
- **Linked PIs:** PI-021 (NEW, cycle 13, auto_safe, applies-this-cycle), PI-022 (NEW, cycle 13, auto_safe, carries-forward documentation-only)
- **Linked regression scenarios:** RS-023 (NEW, cycle 13)

## EV-024 — Cycle-13 inline-review deviation verified (Δ = 0 perfect match); PI-023 codification applied cycle 14 (NEW, cycle 14)

- **Evidence ID:** EV-024
- **Evidence statement:** Cycle-13 deliberately deviated from the Stage 12 / PI-019 reviewer-sub-agent convention by skipping the code-reviewer sub-agent and doing inline review instead. The inline review covered round-4 (retroactive-correction accuracy: cycle-12 backfill verified `136 collected → 134 passed + 1 + 1` matches the cycle-12 closeout memo's `134 passed + 1 + 1` actual exactly) and round-5 (real-world fitness: temporarily broke the cycle-13 forecast to Format B without separate baseline and verified the new test correctly FAILED, then restored). Cycle-13 had Δ = 0 (perfect forecast match); PR #73 merged cleanly at `4da0380`; the cycle-12 −2 forecast-format-labeling delta was closed by PI-021. The deviation was documented as a one-off judgment call in the cycle-13 closeout memo (`memory/2026-07-07-cycle-13-closeout.md` "Code-reviewer sub-agent deviation" section) and the PR #73 body "Inline review (cycle-13, deviates from Stage 12 / PI-019 reviewer-sub-agent convention by design)" section. **PI-023 codifies the deviation** as a documented convention (cycle 14 NEW): inline review is acceptable IF AND ONLY IF (a) no new stages, (b) ≤1 new mechanical test, (c) mechanical substantive change (workflow-doc + ledger + cycle-row backfill only), and (d) inline round-4 + round-5 verification demonstrated in PR body. Reviewer-sub-agent is required if any of: new stage added, ≥2 new tests or any test introducing a new convention, modification to existing tests other cycles depend on, new PI affecting forward-looking forecasts, ≥3 cycles' retroactive corrections touched. PI-023's enforcing test (`tests/dreaming/test_pr_readiness.py::test_pr_change_log_includes_inline_review_deviation_justification_or_reviewer_subagent_run`) scans the most recent cycle row's `Code-reviewer` section for one of the two phrases (`Inline review deviation justification` or `Reviewer-sub-agent run`) and rejects cycles that omit the section or use ambiguous phrasing.
- **Quantitative summary:**
  - Cycle-13 substantive work: 7 files changed (+420 / −4); 1 new test (`test_pr_change_log_forecast_uses_explicit_collected_or_passed_label`); 1 workflow-doc amendment (Stage 0a); 3 ledger entries (PI-021, RS-023, EV-023); 1 retroactive correction (cycle-12 row's forecast line rewritten in Format A).
  - Cycle-13 forecast: `137 collected → 135 passed + 1 + 1` (Format A, `substantive-commit-only` merge-state assumption). Actual: `135 passed + 1 + 1`. **Δ = 0 (perfect match)**.
  - Cycle-13 reviewer sub-agent: NOT spawned (inline review instead, documented as PI-023 codification baseline).
  - Cycle-13 inline-review findings: round-4 (cycle-12 backfill math verified); round-5 (false-positive simulation verified); no latent issues found.
