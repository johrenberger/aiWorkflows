...
- **Actual on `main` post-merge (re-measured at merge SHA `fd822b0`, clean working tree):** **130 passed + 1 skipped + 1 expected-fail-on-main**. The forecast did NOT match (off by +3 passed). The +3 came from `@pytest.mark.parametrize("path", _all_dreaming_files(), ...)` in `tests/dreaming/test_no_hidden_reasoning_capture.py`, which enumerates every file in `.openclaw/dreaming/` and runs three parametrized tests per file. Cycle 11 added 1 NEW file to `.openclaw/dreaming/` — `cycle-11-review-log.md` (committed by reviewer) — and modified 2 existing files (`workflow-nightly-dreaming.md` adding Stage 11 + Stage 12, `proposed-improvements.md` adding PI-018 + PI-019). Only the 1 NEW file was newly enumerated by `_all_dreaming_files()`, contributing +3 parametrized test invocations; the 2 modified files were already present pre-cycle-11 and did not add new parametrized test invocations. Per Stage 11 step 6, the cycle-11 closeout memo (`memory/2026-07-01-cycle-11-closeout.md`) was corrected with the actual measured count and a Forecast-accuracy section.
...
- **Actual on `main` post-merge (re-measured at merge SHA `4da0380`, clean working tree, on `main` branch):** **135 passed + 1 skipped + 1 expected-fail-on-main** (137 tests collected − 1 skipped `test_commits_use_chore_dreaming_prefix` − 1 expected-fail-on-main `test_current_branch_uses_dreaming_prefix`). The forecast matched the actual exactly (Δ = 0). The arithmetic `137 collected → 135 passed + 1 + 1` matches the actual perfectly. **Forecast-accuracy verdict: PERFECT MATCH.** PI-021 (forecast-format-label clarification, Format A explicit arithmetic) and PI-022 (forecast-merge-state-clarification sibling amendment, explicit `substantive-commit-only` merge-state assumption) worked as designed on cycle 13. This is the **first cycle with a Δ = 0 forecast match**, validating PI-021 + PI-022 as a complete solution to the forecast-discipline gaps surfaced across cycles 11-12 (cycle-11 +3 methodology, cycle-12 PR #71 −4 merge-state, cycle-12 PR #72 −2 label-format). Per Stage 11 step 6, the cycle-13 closeout memo (`memory/2026-07-07-cycle-13-closeout.md`) was created with the actual measured count and a cross-cycle forecast-accuracy comparison table showing cycle-13 as the first cycle with a perfect match.
...
- **Actual on `main` post-merge (re-measured at merge SHA `badd6ad`, clean working tree, on `main` branch):** **136 passed + 1 skipped + 1 expected-fail-on-main** (138 tests collected − 1 skipped `test_commits_use_chore_dreaming_prefix` − 1 expected-fail-on-main `test_current_branch_uses_dreaming_prefix`). The forecast matched the actual exactly (Δ = 0). The arithmetic `138 collected → 136 passed + 1 + 1` matches the actual perfectly. **Forecast-accuracy verdict: PERFECT MATCH (second consecutive cycle with Δ = 0).** PI-021 + PI-022 + PI-023 all worked as designed. PI-023 (reviewer-sub-agent convention refinement) was validated by the new test `test_pr_change_log_includes_inline_review_deviation_justification_or_reviewer_subagent_run` correctly enforcing the code-reviewer section convention. Per Stage 11 step 6, the cycle-14 closeout memo (`memory/2026-07-07-cycle-14-closeout.md`) was created with the actual measured count, the cross-cycle forecast-accuracy comparison table, and the cycle-14 inline-review deviation justification.
...
- **Actual on `main` post-merge (re-measured at merge SHA `cf03428`, clean working tree, on `main` branch):** **136 passed + 1 skipped + 1 expected-fail-on-main** (138 tests collected − 1 skipped `test_commits_use_chore_dreaming_prefix` − 1 expected-fail-on-main `test_current_branch_uses_dreaming_prefix`). The forecast matched the actual exactly (Δ = 0). The arithmetic `138 collected → 136 passed + 1 + 1` matches the actual perfectly. **Forecast-accuracy verdict: PERFECT MATCH (THIRD consecutive cycle with Δ = 0).** PI-021 + PI-022 + PI-023 + PI-009 all worked as designed. PI-009 (generalize PI-008 to SGP) was validated by `make sgp-validate` correctly catching a deliberately introduced ruff violation at step 1/4 with the same diagnostic CI would produce (verified during cycle-15 development). SGP-side validation on actual `main` at `cf03428`: ruff clean, mypy clean (`Success: no issues found in 23 source files`), pytest 429 passed, branch coverage `TOTAL 1913 115 660 68 92.2%` (above 90% gate). Per Stage 11 step 6, the cycle-15 closeout memo (`memory/2026-07-07-cycle-15-closeout.md`) was created with the actual measured count, the cross-cycle forecast-accuracy comparison table, and the cycle-15 inline-review deviation justification.
...
merged to `main` via PR #75 (merge SHA `cf03428`). Cycle-15 branch deleted via PR #75 `--delete-branch` cleanup. Closeout memo at `memory/2026-07-07-cycle-15-closeout.md` documents the **third consecutive cycle with Δ = 0 forecast match**. PI-009 (generalize PI-008 to SGP) was validated by `make sgp-validate` correctly catching a deliberately introduced ruff violation at step 1/4 with the same diagnostic CI would produce; the cycle-15 PR body included an "Inline review deviation justification" section listing PI-023 criteria (a)-(d) and demonstrating round-4 + round-5 verification.
