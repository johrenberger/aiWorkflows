...
- **Pre-push:** `make dreaming-validate` on the cycle-10 branch — expected: **125 passed, 0 failed, 0 skipped** (one new test added, no skips).
- **Post-merge on main:** the new test continues to pass because the cycle-10 commit is in sync with the working tree.
