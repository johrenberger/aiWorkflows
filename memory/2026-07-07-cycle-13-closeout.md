...
3. **Inline-review vs reviewer-sub-agent deviation (cycle 14 candidate).** Cycle-13 deviated from Stage 12 / PI-019 by skipping the code-reviewer sub-agent and doing inline review instead. The deviation is documented; if a future cycle finds that the inline-review approach misses latent issues, the convention should be re-adopted. Possible cycle-14 PI: PI-023 (reviewer-sub-agent convention refinement — codify when inline-review is acceptable vs when the reviewer-sub-agent is required).
