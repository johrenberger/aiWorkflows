...
def test_final_report_renders_queue_items_with_path_fallback(tmp_path):
    ...
    (tmp_path / "coverage_baseline.json").write_text(json.dumps([{"path": "src/Foo.java", "line_coverage": 80.0, "branch_coverage": 70.0}]), encoding="utf-8")
    ...
    (tmp_path / "adapter_detections.json").write_text(json.dumps([{"language": "java", "adapter": "java_junit", "confidence": 0.5, "evidence": ["pom.xml"]}]), encoding="utf-8")
    (tmp_path / "commands_discovered.json").write_text(json.dumps([{"language": "java", "kind": "test", "command": "mvn test"}]), encoding="utf-8")
    ...
    (mutation_dir / "mutation_tool_detection.json").write_text(json.dumps({"java": {"tool": "pitest", "available": True}}), encoding="utf-8")
    ...
    assert "## Commands Discovered" in report
    assert "mvn test" in report
    assert "coverage reports were not discovered in this run" not in report
